package cn.jackuxl.qforum.mapper

import cn.jackuxl.qforum.model.App
import cn.jackuxl.qforum.model.Board
import org.apache.ibatis.annotations.Insert
import org.apache.ibatis.annotations.Select
import org.springframework.stereotype.Component

@Component
interface AppMapper {
    @Insert("insert into qf_app(id,name,postTime,up,publisherId,downloadUrl,iconUrl,slogan,version,packageName,description) values(#{id},#{name},#{postTime},#{up},#{publisherId},#{downloadUrl},#{iconUrl},#{slogan},#{version},#{packageName},#{description})")
    fun postApp(app: App): Int

    @Select("select * from qf_app where id=#{param1};")
    fun getAppById(id: Int): App

    @Select("select * from qf_app order by id desc")
    fun listApps(): List<App>
}