...
class FileController {
    ...
    fun uploadImage(
        ...
        val rootPath =
            File("upload/images/${LocalDate.now().year}/${LocalDate.now().monthValue}/${LocalDate.now().dayOfMonth}")
        rootPath.mkdirs()
    ...
    fun uploadAPK(file: MultipartFile): ResultEntity<Map<String, String>> {
        ...
        val rootPath =
            File("upload/apks/${LocalDate.now().year}/${LocalDate.now().monthValue}/${LocalDate.now().dayOfMonth}")
        rootPath.mkdirs()
    ...
    @RequestMapping(
        value = ["/upload/apks/{year}/{month}/{day}/{fileName}"],
